...
function normalizeColor(source) {
    ...
    if (color) {
        ...
        return formatRGB(source).toUpperCase();
    } else if('transparent' == source) {
        return 'rgba(0, 0, 0, 0)'.toUpperCase();
    }
//        source = source.replace(/#([\da-f])([\da-f])([\da-f])\b/g, '#$1$1$2$2$3$3')
//        .replace(source, function (a, start, color, reset) {            
//            if (color) {
//                if (maybeColor) {
//                    if (/^hsl/i.test(color)) {
...
(function (window) {
    ...
    var QUnit = {
        ...
        colorEqual:function (actual, expected, message) {
            equal(normalizeColor(actual), normalizeColor(expected), message);
        },

        colorNotEqual:function (actual, expected, message) {
            notEqual(normalizeColor(actual), normalizeColor(expected), message);
        },
