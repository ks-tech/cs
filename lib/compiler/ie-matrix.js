function updateMatrixStyle(s){
	...
	while(i--){
		...
		if(/(?:-\w+-)?transform/.test(n)){
			...
			value = toMatrix(value).map(function(i){return i.toFixed(16).replace(/\.?0+$/,'')}).join(',');
			if(/-moz-/.test(n)){
...
MatrixGenerator.rotateZ = MatrixGenerator.rotate
...
exports.updateMatrixStyle = updateMatrixStyle
