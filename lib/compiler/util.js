...
function CSSManager(resource,cssom,index){
	...
	this.setup = function(pid,selectors,appendInfo){
		if(clean(selectors,cache,pid,appendInfo)){
			...
			selectorText = selectorText.replace(/\:(?:first-letter|first-line)\b/g,'');
			if(ieversion<'8'){
				cssom.insertRule(selectorText+"{"+(appendInfo||'')+getPluginStyle(pid)+":expression(CS(this,'"+pid+"'));}",index+1);
			}else if(ieversion === '8'){
				var selector2 = selectorText.replace(/\:(?:hover|active|focus)\b/g,'');
				if(selector2!=selectorText){
					appendInfo && cssom.insertRule(selectorText+"{"+appendInfo+"}",index+1);
					cssom.insertRule(selector2+"{behavior:url("+htc+");}",index+1);
				}else{
					cssom.insertRule(selector2+"{"+(appendInfo||'')+"behavior:url("+htc+");}",index+1);
				}
			}else{
				cssom.insertRule(selectorText.join(',')+"{"+(appendInfo||'')+"-ms-behavior:url("+htc+");}",index+1);
			}
		}
		return this;
	}
