	...
	this.setup = function(pid,selectors,appendInfo){
		if(clean(selectors,cache,pid,appendInfo)){
			if(ieversion<'8'){
				...
				cssom.insertRule(selectors.join(',')+"{"+(appendInfo||'')+"behavior:url(/static/cs.htc);}",index+1);
			}else{
				cssom.insertRule(selectors.join(',')+"{"+(appendInfo||'')+"-ms-behavior:url(/static/cs.htc);}",index+1);
			}
		}
		return this;
	}
