...
function CSSManager(resource,cssom,index){
	...
	this.setup = function(pid,selectors,appendInfo){
		if(clean(selectors,cache,pid)){
			if(ieversion<'8'){
				cssom.insertRule(selectors.join(',')+"{"+(appendInfo||'')+getPluginStyle(pid)+":expression(CS(this,'"+pid+"'));}",index+1);
			}else if(ieversion === '8'){
				cssom.insertRule(selectors.join(',')+"{"+(appendInfo||'')+"behavior:url(/cs.htc);}",index+1);
			}else{
				cssom.insertRule(selectors.join(',')+"{"+(appendInfo||'')+"-ms-behavior:url(/cs.htc);}",index+1);
			}
		}
		return this;
	}
