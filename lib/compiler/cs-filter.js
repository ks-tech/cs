...
alert(s.textIndent)
 */
function setupCS(rbs, cssPattern, htmlPattern, scriptBase){
	...
	if(htmlPattern){
		rbs.addTextFilter(htmlPattern,function(resource,text){
			return htmlFilter(text,scriptBase+'cs.js');
		})
	}
...
//	})
	//文本预处理
	rbs.addTextFilter(cssPattern,function(resource,text){
		resource.cs = {config :{},cache:{}};
		return text;
	})
	//文件关系预处理 （@import @media）
	rbs.addDOMBuilder(cssPattern,function cssomBuilder(resource,data){
		//标准化 color
...
function htmlFilter(text,scriptImpl){
	var needImpl = true;
	return text.replace(/<script\b[^>]*\/>|<script\b[^>]*>[\s\S]*?<\/script>|(<link\s+[\s\S]+?>(?:\s+|<!--[\s\S]*?-->|<link\s+[\s\S]+?>))/g,function(a,links){
		if(links){
			links.replace(/<!--[\s\S]*?-->/g,'');
			...
			if(cssLinks.length){
				...
				otherLinks.push('<script>CS'+cssLinks.join('')+'</script>')
			}
...
var ie7SelectorFilter = require("./cs-ie78-selector-filter").ie7SelectorFilter
var ie8SelectorFilter = require("./cs-ie78-selector-filter").ie8SelectorFilter
