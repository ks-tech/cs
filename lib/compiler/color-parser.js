...
buf.push('(?:rgb|hsl)a?\\s*\\(\\s*\\d+%?\\s*,\\s*\\d+%?\\s*,\\s*\\d+%?\\s*(?:,\\s*[\\d\\.]+)?\\s*\\)')
var colorPattern = new RegExp(
	"\\b(background|(?:[\\w\\-]+\\-)?color|border|border-\\w+)\\s*\\:" +
	"|'[^']*'|\"[^\"]*\"" +
	"|\\b("+buf.join('\\b|')+")" +
	"|([\\}{;])",'ig');
...
function normalizeColor(source){
	//console.log(source)
	source = source.replace(/#([\da-f])([\da-f])([\da-f])\b/ig,'#$1$1$2$2$3$3');
	var maybeColor;
	var color = colorMap[source.toLowerCase()];
	if(color){
		return color;
	}
	source = source.replace(/#[\da-f]{6}/g,function(a){return a.toUpperCase()})
				.replace(colorPattern,function(a,start,color,reset){
					//console.log(start,color,reset)
					if(color){
						if(maybeColor){
							if(/^hsl/i.test(color)){
