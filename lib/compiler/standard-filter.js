...
function buildFilterByPrefix(sourcePrefix,implPrefix){
	...
	var isIE = /^-ie\d?-$/.test(sourcePrefix[0]);
	...
	return function(resource,cssom,rule,index){
		if(rule.type == 8){
			//console.log(rule.type,rule._vendorPrefix,index)
			if(rule._vendorPrefix){
				if(rule._vendorPrefix != implPrefix || isIE){
					cssom.deleteRule(index);
				}
			}else{
				rule._vendorPrefix = implPrefix
			}
			return;
		}
		var s = rule.style;
