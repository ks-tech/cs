...
function ie6Filter(resource,cssom,rule,index){
	...
	//join-muti-classes
	walkSelector(selectorData,joinMutiClassesSelector);
	...
	walkSelector(selectorData,buildClassesInfoCollector(mutiClasses,attributeClasses,nthClasses,notClasses))
	...
	walkNode(selectorData,function(group,node,index){//sort
		...
		if(node[0] == '>'){
			node[0] = ' ';
			var parentId = '.attr-data-parent-'+encodeSelector(prefix)+'__';
		...
		if(classIndex){
			...
			if(parentId){
				childIdsMap[className] = parentId
			}
		}
 ...
 */
function buildClassesInfoCollector(mutiClasses,attributeClasses,nthClasses,notClasses){
