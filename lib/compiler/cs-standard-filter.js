...
function buildFilterByPrefix(sourcePrefix,realPrefixMap,validPropertyMap){
	...
	var prefixPattern = new RegExp(sourcePrefix.join('|'))
	...
	return function(resource,cssom,rule,index){
		...
		for(var i=0;i<len;i++){
			...
			if(n in validPropertyMap){
				...
				if(!value || prefixIndex <= value[2]){
					valueMap[n] = [s.getPropertyValue(n),s.getPropertyPriority(n),prefixIndex];
				}
			}else{
		...
		for(var n in valueMap){
			...
			value = value[0];
			n = realPrefixMap[n] || n;
			value = value.replace(/((?:-\w+)+-|\b)((?:repeating-)?(?:radial|linear)-gradient)(?=\()/,function(a,prefix,key){
				var real = realPrefixMap[key];
				if(prefix && findPrefixIndex(prefix) < 0){
					//这里没有了优先级控制，也不应该有前缀
					//console.warn('!!!')
				}
				return real || key;
