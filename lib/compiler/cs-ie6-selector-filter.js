...
function ie6SelectorFilter(resource,cssom,rule,index){
	...
	while(i){
		var refEncoded = thisAndReference[--i]
		var thiz = thisAndReference[--i]
		if(refEncoded != (refEncoded = refEncoded.replace(/^attr-data_2D(?=[ag]s_2D|child_2D)/,''))){
			var refSource = decodeURIComponent(refEncoded.replace(/_/g,'%'));
			var p = refSource.indexOf('-');
			
			var type = refSource.substring(0,p);
			var refSelector = refSource.substring(p+1);
			//add this updater and attribute
			setup('update',
					[thiz],'relation-classes',refEncoded.substr(refEncoded.indexOf('_')+3),
					'cs-update-'+refEncoded+';cs-update-relation=1;'+id+':1;');
			//add parent css attribute
			cssom.insertRule(refSelector+'{'+id+'-relation:1;}',index+1)
		}
	}
}
	...
	return function(group,node,index){
		...
		if(className){
			...
			if(psClassNames.length){
				...
				//参照系
				var csAttribute = null;
				var nocsClassName = removeClass(className,/^attr-data_2D(?:as|gs|child)_2D__/,[]);
				nocsClassName != className && psClassNames.some(function(c,index){
					if(/^attr-data_2D(?:as|gs|child)_2D__$/.test(c)){
						csAttribute = c + encodeSelector(parentPrefix)
						if(/^attr-data_2[ag]s/.test(csAttribute)){
							parentPrefix = groupResult.pop(),groupResult.join('');
						}
						psClassNames[index] = csAttribute ;
						return true;
					}
				})
				...
				if(csAttribute){
					...
					if(nocsClassName.indexOf('--')>0){
						mutiSelectors.push(prefix + nocsClassName.replace(/^\..*--/,'.') +postfix)
						mutiClasses.push(nocsClassName.substr(1))
					}
					var nocsFullClassName = prefix + nocsClassName +postfix;
					thisAndReference.push(nocsFullClassName,csAttribute);
					//rebuild className(sort顺序不会影响,所以能直接替换)
					className = className.replace(/(.|--)attr-data_2D(?:as|gs|parent)_2D__(?=--|$)/,"$1"+csAttribute)
				}
