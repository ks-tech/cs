...
var normalizeIE8Pseudo = require('./cs-ie78-selector-filter').normalizeIE8Pseudo
var pseudoElementToEnd = require('./cs-ie78-selector-filter').pseudoElementToEnd

function ie6SelectorFilter(resource,cssom,rule,index){
	var selectorData = parseSelectors(rule.selectorText);
	var hasPSE = false;//pseudoElementToEnd(selectorData)
	// prepare for >~+
	...
	while(i--){
		var item = result[i];
		var ownerItem = item.replace(/>\.(?:before|after)__$/,'');
		if(ownerItem != item){
			css.setup(item.slice(ownerItem.length+2,-2),[ownerItem])
		}
	}
	//if(hasPSE){//>init defualt
	//	css.setup('dc',['.before__','.after__'],'cs-update-gen:1');
	//}
