...
var tracePC = true;
var configNth = require('./nth').configNth;
var computeNth = require('./nth').computeNth;
...
//		console.log('nth-classes:',style.cssText)
	...
	setup:function(el){//可选
		var cn = computeNth(el);
		updateMultiClasses(cn,el)
		//el.className = out.join(' ');
		//console.log('nth',out.join(' '),el.className)
	},
	//visit:function(){}
...
var listenerMap = {
	...
	"cs-linear-gradient":function(el,gradient){
		FilterHelper(el).setLinearGradient(gradient).update();
	}
}
CS.addPlugin({id:'listener',//attribute query plugin
	setup:function(el,config){//可选
	},
	visit:function(el,config){
		var cs = el.currentStyle;
		var transition = cs['transition-property'];
		transition && vistTransition(el,config,transition);
		for(var n in listenerMap){
			var cv = cs[n];
			if(cv != config[n]){
				listenerMap[n](el,config[n] = cv);
			}
		}
...
var  parentIdMap = {}
CS.addPlugin({id:'child',//attribute query plugin
	config:function(style){
//		console.log('nth-classes:',style.cssText)
