...
exports .vistTransition = vistTransition;
var extStyleMap = {'opacity':0}
function vistTransition(el,transition,config){
	var cs = el.currentStyle;
	var transitionConfig = config.transition;;
	config['transition-property'] = Math.random();//让下次轮循能够进入处理函数
	if(!transitionConfig){
		//delay
		config.transition = {}
		return;
	...
	if(backup){
		tryTransition(el,backup,transitionConfig);
	}else{
		transitionConfig.backup = backupProperties(transition,currentStyle)
	}
}
