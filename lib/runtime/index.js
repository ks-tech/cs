	...
	update: function(el,config,transform){
		...
		if(m){
			...
			setTimeout(function(){
				var x = +m[5];
				var y = +m[6];
				var offsetLeft = x+(el.clientWidth - el.offsetWidth) / 2;
				var offsetTop = y+(el.clientHeight - el.offsetHeight) / 2;
...
//				rs.marginTop = (parseInt(rs.marginTop)||0) -oldOffset[1] + offsetTop + 'px'; 
				...
				//console.log([rs.left,rs.top],[x,y,offsetLeft,offsetTop],'\n',el.clientWidth - el.offsetWidth)
				config.csTransformOffset = [offsetLeft,offsetTop];
				rs = null;
				el = null;
			},0);
		}else{
			ElementExtension(el).setTransform(null);
			var oldOffset = config.csTransformOffset || [0,0];
			rs.posLeft = rs.posLeft -oldOffset[0];
			rs.posTop = rs.posTop -oldOffset[1]; 
		}
	}
},'onchange','transform')
