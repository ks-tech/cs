			...
			if(s){
				...
				if(s.selectorText.toLowerCase() == 'head cs-config'){
					for(var n in pluginRuntimeMap){
						...
						if(n.config){
							setPropertyListener(n.config(s.style));
						}
					}
				}
			}
...
CS.addPlugin({id:'poll',//attribute query plugin
	setup:function(el,config){//可选
	},
	visit:function(el,config){
		var cs = el.currentStyle;
		if(cs['cs-alive']!='false'){
			for(var n in pollMap){
				var cv = cs[n];
				if(cv != config[n]){
					pollMap[n](el,config[n] = cv,config);
				}
			}
		}
	},
...
/*--------------- utils ----------*/
function setPropertyListener(propertyMap){
	if(propertyMap){
		var buf = [cpid] 
...
//textKashida:mc
//textKashidaSpace:dc
//pageBreakBefore:attr
//textUnderlinePosition:child
//pageBreakAfter:nth
//layoutFlow:poll
//
//rubyOverhang:matrix
//rubyAlign:box
//rubyPosition:gradient
