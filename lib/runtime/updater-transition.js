...
function waitTransition(el,runtimeStyle,changeMap,config,timing,duration,delay){
	...
	for(var p in changeMap){
		if(/^(?:width|height)$|^padding-|^border-[\w\-]*width/.test(p)){
	...
	waiting.timeout = setTimeout(function(){
		...
		config.task = startTransition(runtimeStyle,transitionMap,onComplete,timing,duration)
	},delay);
 ...
 */
function startTransition(runtimeStyle,transitionMap,onComplete,
	transform,duration){
	function step(rate){
		for(var n in transitionMap){
			...
			// ElementExtension(el).setOpacity()
			//	//console.log(runtimeStyle.filter)
			//}else{
				try{
					//if(rate == 1)console.log(n,'->',v)
					runtimeStyle[n] = v;
				}catch(e){console.log('style error:',n,v);throw e;}
			//}
		}
	}
