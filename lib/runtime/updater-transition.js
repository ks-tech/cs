...
function tryTransition(el,currentStyle,config){
	...
	if(changeMap){
		...
		var delay= currentStyle['transition-delay'];
		if(/\ds$/i.test(delay)){
			delay = parseFloat(delay) * 1000;
		}else{
			delay = parseFloat(delay) || 250;
		}
		waitTransition(el,runtimeStyle,changeMap,config,timing,duration,delay||250)
	}
}
...
function bindTransition(fn,op){
	return function(rate,n){
		try{
			op(fn(rate),n)
		}catch(e){console.log('transition error:',rate,fn(rate),op);throw e;}
	}
}
function buildOffsetImpl(p,op,np){
	...
	if(/transform/.test(p)){
		...
		var postfix = np.replace(/[\d\.\-]+|auto/i,'');
